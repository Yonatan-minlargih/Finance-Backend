...
import com.financial.corefinance.dto.request.JournalReversalRequest;
...
public class JournalController {
    ...
    public ResponseEntity<JournalHeaderResponse> reverseJournal(
            @Parameter(description = "Journal ID") @PathVariable UUID journalId,
            @Parameter(description = "Reversal reason (query param)") @RequestParam(required = false) String reversalReason,
            @RequestBody(required = false) JournalReversalRequest body) {
        String reason = resolveReversalReason(reversalReason, body);
        log.info("Reversing journal with ID: {} for reason: {}", journalId, reason);

        JournalHeader reversalJournal = postingEngineService.reverseJournal(journalId, reason);
        JournalHeaderResponse response = journalMapper.toJournalHeaderResponse(reversalJournal);

        return ResponseEntity.ok(response);
    }

    private static String resolveReversalReason(String reversalReason, JournalReversalRequest body) {
        if (reversalReason != null && !reversalReason.isBlank()) {
            return reversalReason.trim();
        }
        if (body != null) {
            if (body.getReversalReason() != null && !body.getReversalReason().isBlank()) {
                return body.getReversalReason().trim();
            }
            if (body.getReason() != null && !body.getReason().isBlank()) {
                return body.getReason().trim();
            }
        }
    ...
    // @PreAuthorize("hasRole('ACCOUNTANT') or hasRole('FINANCE_MANAGER') or
    // hasRole('AUDITOR')")
    ...
    public ResponseEntity<Page<JournalHeaderResponse>> getAllJournals(
        ...
        Page<JournalHeader> journals = postingEngineService.getAllJournals(pageable);
    ...
    public ResponseEntity<Page<JournalHeaderResponse>> searchJournals(
        ...
        Page<JournalHeader> journals = postingEngineService.searchJournals(search, pageable);
        Page<JournalHeaderResponse> response = journals.map(journalMapper::toJournalHeaderResponse);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<List<JournalHeaderResponse>> getJournalsByStatus(
        ...
        List<JournalHeader> journals = postingEngineService.getJournalsByStatus(status);
    ...
    @GetMapping("/lines/account/{accountId}")
    @Transactional(readOnly = true)
    @Operation(summary = "Get posted lines for account", description = "Retrieves all posted journal lines mapped to a specific account (Ledger Posting Activity drilldown)")
    public ResponseEntity<List<JournalLineResponse>> getPostedLinesByAccount(
        ...
        List<JournalLineResponse> response = postingEngineService.getPostedLinesByAccount(tenantId, accountId)
                .stream()
                .map(journalMapper::toJournalLineResponse)
                .collect(Collectors.toList());
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<List<JournalLineResponse>> getUnreconciledPostedLines() {
        List<JournalLineResponse> response = postingEngineService.getUnreconciledPostedLines()
                .stream()
                .map(journalMapper::toJournalLineResponse)
                .collect(Collectors.toList());
        return ResponseEntity.ok(response);
    ...
    public ResponseEntity<Void> setJournalLineReconciliationStatus(
        ...
        postingEngineService.markJournalLineReconciled(lineId, reconciled);
