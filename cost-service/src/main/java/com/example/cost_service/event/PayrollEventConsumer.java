...
public class PayrollEventConsumer {
    ...
    public void handleExternalPayrollApproved(PayrollEventDto event) {
        try {
            ...
            costRecord.setLaborCost(event.getTotalGross());
            costRecord.setMaterialCost(BigDecimal.ZERO);
            costRecord.setOverheadCost(BigDecimal.ZERO);
            ...
            calculatedEvent.setPeriodId(resolvedPeriodId);
            calculatedEvent.setLaborCost(event.getTotalGross());
            calculatedEvent.setMaterialCost(BigDecimal.ZERO);
            calculatedEvent.setOverheadCost(BigDecimal.ZERO);
            calculatedEvent.setTotalCost(event.getTotalGross());
            calculatedEvent.setCalculatedAt(LocalDateTime.now());
