...
@Table(
        ...
        uniqueConstraints = {
                @UniqueConstraint(
                        name = "uq_numbering_series_tenant_code",
                        columnNames = {"tenant_id", "series_code"}
                )
        }
)
@Data
...
public class NumberingSeries extends BaseEntity {
    ...
    // ✅ FIX: renamed column to avoid MySQL reserved keyword
    ...
    public void generateFormat() {
        if (format == null || format.isEmpty()) {
            ...
            if (prefix != null && !prefix.isEmpty()) {
                formatBuilder.append(prefix).append(delimiter);
            }

            formatBuilder.append("{NUMBER:").append(numberLength).append("}");

            if (suffix != null && !suffix.isEmpty()) {
                formatBuilder.append(delimiter).append(suffix);
            }
    ...
    public String generateNextNumber() {
        ...
        if (prefix != null && !prefix.isEmpty()) {
            numberBuilder.append(prefix).append(delimiter);
        }

        if (format != null) {
            String formatted = format.replace(
                    "{NUMBER:" + numberLength + "}",
                    String.format("%0" + numberLength + "d", currentNumber)
            );

            formatted = formatted.replace("{YYYY}", String.valueOf(Year.now().getValue()));
            formatted = formatted.replace("{MM}", String.format("%02d", LocalDate.now().getMonthValue()));
            formatted = formatted.replace("{DD}", String.format("%02d", LocalDate.now().getDayOfMonth()));
        ...
        if (suffix != null && !suffix.isEmpty()) {
            numberBuilder.append(delimiter).append(suffix);
        }
